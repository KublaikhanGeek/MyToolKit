...
unsigned long TimerManager::addTimer(unsigned int interval,
                                     std::function<void()> action,
                                     bool isRepeat, bool isNoDelay, int id) {
  Timer event(interval, this->timeline_, std::move(action), isRepeat,
              isNoDelay);
  event.m_id =
      (id != -1) ? static_cast<unsigned long>(id) : this->autoIncrementId_++;
...
void TimerManager::deleteTimer(unsigned long &timerId) {
  if (timerId < 1) {
    return;
  }
