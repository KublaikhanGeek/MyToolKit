...
int create_tpool(tpool_t **pool, size_t max_thread_num)
{
     ...
     {
          printf("in %s,init thread id failed,errno = %d,explain:%s", __func__, errno,
                 strerror(errno));
          exit(-1);
     }
     ...
     {
          printf("in %s,initial mutex failed,errno = %d,explain:%s", __func__, errno,
                 strerror(errno));
          exit(-1);
     }
     ...
     {
          printf("in %s,initial condition variable failed,errno = %d,explain:%s",
