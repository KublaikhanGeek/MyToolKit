...
static const unsigned char data_ascii2bin[128] = {
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xE0, 0xF0, 0xFF, 0xFF, 0xF1, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xE0, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x3E, 0xFF, 0xF2, 0xFF, 0x3F, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x3B, 0x3C,
    0x3D, 0xFF, 0xFF, 0xFF, 0x00, 0xFF, 0xFF, 0xFF, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A,
...
int base64_encode(unsigned char* in, unsigned char* out, int out_len)
{
    ...
    for (i = 0; i < in_len; i++)
    {
        if (in[i] > 127)
        {
            dzlog_error("can not encode Non-ASCII characters");
            return -1;
        }
    }
    ...
    {
        dzlog_error("no enough buffer space");
        return -1;
    }
    out[encode_len] = '\0';
...
int base64_decode(unsigned char* in, unsigned char* out, int out_len)
{
    ...
    for (i = 0; i < nLoop; i += 4)
    {
        for (j = 0; j < (int)(sizeof(b) / sizeof(b[0])); ++j)
        {
    ...
    if (in[in_len - 1] == '=' && in[in_len - 2] != '=')
    {
        if (in[i] >= table_count || in[i + 1] >= table_count || in[i + 2] >= table_count)
        {
            dzlog_error("bad base64 string");
            return -1;
        }
